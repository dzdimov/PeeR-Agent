...
import { getCoverageAnalysis, runEslintAnalysis, } from './coverage-analyzer.js';
 ...
 */
function findFilesRecursive(dir, filterFn) {
    const results = [];
    try {
 ...
 */
function isTestFileCheck(filePath) {
    const basename = path.basename(filePath);
 ...
 */
export async function findSourceFiles(repoPath) {
 ...
 */
export async function findTestFiles(repoPath) {
 ...
 */
export function findFilesWithoutTests(sourceFiles, testFiles) {
 ...
 */
export async function scanCodebaseBaseline(repoPath) {
 ...
 */
export async function generateBaselineTestSuggestions(repoPath, filesWithoutTests, maxSuggestions = 10) {
    const suggestions = [];
    ...
    for (const file of filesWithoutTests.slice(0, maxSuggestions)) {
        ...
        try {
            ...
            if (functionNames.length === 0)
                continue;
            const testCode = generateTestTemplate(frameworkInfo.framework, file, content.substring(0, 2000), // First 2000 chars for context
            functionNames);
            suggestions.push({
                forFile: file,
 ...
 */
export function formatBaselineIssues(issues, filter) {
    const lines = [];
...
//# sourceMappingURL=baseline-scanner.js.map