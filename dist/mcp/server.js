...
import { MCP_SERVER_NAME, MCP_SERVER_VERSION, TOOL_DESCRIPTIONS, TOOL_SCHEMAS, } from './constants.js';
// Create MCP server
 ...
 */
server.tool('analyze', TOOL_DESCRIPTIONS.ANALYZE, TOOL_SCHEMAS.ANALYZE, async (args) => {
 ...
 */
server.tool('saveAnalysisResults', TOOL_DESCRIPTIONS.SAVE_RESULTS, TOOL_SCHEMAS.SAVE_RESULTS, async (args) => {
    return await saveResultsTool.execute(args);
});
/**
 * dashboard - Start the web dashboard (same as CLI 'pr-agent dashboard')
 */
server.tool('dashboard', TOOL_DESCRIPTIONS.DASHBOARD, TOOL_SCHEMAS.DASHBOARD, async (args) => {
