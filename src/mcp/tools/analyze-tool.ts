...
import type { McpToolResponse, AnalysisOutputOptions } from '../types.js';
import {
  GitService,
  DiffParserService,
...
export class AnalyzeTool {
  ...
  async execute(args: AnalyzeToolArgs): Promise<McpToolResponse> {
    ...
    try {
      ...
      if (verbose) {
        console.error(
          `[MCP Server] Config loaded: peerReview.enabled=${peerReviewEnabled}, archDocs=${args.archDocs !== false}`
        );
      }

      const ticketRefs = TicketExtractorService.extractTicketReferences(
        title,
        currentBranch,
        commitMessages,
      ...
      if (verbose) {
        ...
        console.error(
          `  - prompt count: ${analysisResult.mode === 'prompt_only' ? analysisResult.prompts.length : 'N/A'}`
        );
      ...
      let peerReviewError: string | undefined;
      if (peerReviewEnabled) {
        try {
          const peerReviewResult = await PeerReviewService.runPeerReview({
            ...
            diff,
            title,
            prAnalysisResult: analysisResult as any,
            verbose,
            workDir,
          });
      ...
      // Format output
      const outputOptions: AnalysisOutputOptions = {
        verbose,
        peerReviewEnabled,
        peerReviewError,
