...
export function classifyProject(
  ...
  const businessLogicSignals: string[] = [];
  const qaTestingSignals: string[] = [];
  ...
  for (const file of changedFiles) {
    ...
    if (patch) {
      ...
      const businessKeywordCount = BUSINESS_LOGIC_PATTERNS.contentKeywords.filter(
        keyword => patch.includes(keyword)
      ).length;
      
      // Count QA/testing keywords
      const qaKeywordCount = QA_TESTING_PATTERNS.contentKeywords.filter(
        keyword => patch.includes(keyword)
      ).length;
  ...
  let projectType: 'business_logic' | 'qa_testing' | 'mixed' | 'unknown';
  ...
  switch (projectType) {
    case 'business_logic':
      recommendations.push(
        '📋 **Business Logic Project Detected** - Focus on architecture and data flow review',
        '🔒 Ensure proper input validation and error handling for business rules',
        '⚡ Consider performance implications for data processing and API endpoints',
        '🔐 Review authentication and authorization for sensitive business operations',
        '📊 Verify data model changes are properly migrated and validated'
      );
    ...
    case 'qa_testing':
      recommendations.push(
        '🧪 **QA/Testing Project Detected** - Focus on test quality and coverage',
        '✅ Ensure tests are comprehensive and cover edge cases',
        '🎯 Verify test assertions are meaningful and specific',
        '♻️ Check for test maintainability and clear test descriptions',
        '🚀 Consider test execution time and potential for flakiness'
      );
    ...
    case 'mixed':
      recommendations.push(
        '🔀 **Mixed Project Type** - Changes span both business logic and tests',
        '🔄 Ensure business logic changes are properly covered by test changes',
        '⚖️ Verify test changes reflect the business logic modifications',
        '📝 Consider separating business logic and test changes into separate commits for clarity'
      );
      break;

    case 'unknown':
      recommendations.push(
        '❓ **Project Type Unknown** - Unable to determine primary focus',
        '🔍 Consider adding more context to file organization',
        '📚 Review if changes follow project structure conventions'
      );
      break;
  }
 ...
 */
export function formatClassification(classification: ProjectClassification): string {
...
export function createProjectClassifierTool() {
  return new DynamicStructuredTool({
    ...
    description: 
      'Classifies the project type (business logic vs QA/testing) based on changed files. ' +
